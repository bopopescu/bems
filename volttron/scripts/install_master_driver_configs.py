...
description = """
Updates the contents of the Main Driver configuration store with a set of
configurations in a directory. This is designed to work with the output of
the update_main_driver_config.py script.
...
be named "devices/campus/building/my_device" when added to the Main Driver
configuration store.
...
By default this will delete the old main driver configuration store before
...
def install_configs(input_directory, keep=False):
    ...
    keystore = KeyStore()
    agent = Agent(address=get_address(), identity="main_driver_update_agent",
    ...
    if not keep:
        print "Deleting old Main Driver store"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--keep-old', action="store_true",
                        help="Do not remove existing device driver and registry files from the Main Driver configuration store.")
